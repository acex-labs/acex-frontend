    ...
    <Modal
      ...
      footer={
        <>
          ...
          <button
            onClick={() => mutation.mutate({ name: name.trim(), active })}
      ...
      <FormField label="Status">
        <div className="flex items-center gap-2 mt-0.5">
          ...
          <button
            ...
            className={`px-3 py-1 rounded text-[11px] border transition-colors ${
              !active
                ? 'bg-surface-hi border-edge text-content'
                : 'border-edge text-subtle hover:text-content'
            }`}
          >
            Inactive
...
function EditModal({ customer, onClose, onSuccess }) {
  ...
  const mutation = useMutation({
    mutationFn: (data) => updateCustomer(customer.name, data),
    onSuccess: () => { onSuccess(); onClose() },
  })
    ...
    <div className="border-t border-edge bg-surface shrink-0">
      ...
      <div className="px-4 py-3 space-y-1.5">
        <div className="flex gap-3">
          <span className="w-16 shrink-0 text-[11px] text-subtle font-mono">active</span>
          <span className="text-[11px] text-content">{String(customer.active)}</span>
        </div>
        <div className="flex gap-3">
          <span className="w-16 shrink-0 text-[11px] text-subtle font-mono">phase</span>
          <span className="text-[11px] text-content">{customer.phase || '—'}</span>
        </div>
        {customer.path && (
          <div className="flex gap-3">
            <span className="w-16 shrink-0 text-[11px] text-subtle font-mono">path</span>
            <span className="text-[11px] text-content font-mono">{customer.path}</span>
          </div>
        )}
      </div>
    </div>
...
export default function CustomersPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['naas-customers'] })
      setSelected(null)
    },
  })
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      <PageHeader
        title="Customers"
        description={customers.length > 0 ? `${customers.length} customers` : undefined}
        actions={
          <button
      ...
      <div className="flex-1 overflow-hidden flex flex-col min-h-0">
        <div className="flex-1 overflow-auto">
                ...
                : customers.map(c => (
                    <tr
                      ...
                      className={[
                        ...
                        selected?.name === c.name ? 'bg-brand/5' : 'hover:bg-surface-hi',
                      ].join(' ')}
                    >
          ...
          {!isLoading && customers.length === 0 && (
            <div className="px-6 py-8 text-xs text-subtle">No customers found.</div>
          )}
        </div>
